...
With this `mloa` dataframe, remove the NAs (here designated with -99.9 or -999.9) in rel_humid, temp_C_2m, and windSpeed_m_s. Use the [README]("https://raw.githubusercontent.com/gge-ucd/R-DAVIS/master/data/mauna_loa_README.txt") file associated with the Mauna Loa dataset to determine in what time zone the data are reported. Generate a column called "datetime" using the year, month, day, hour24, and min columns. Next, create a column called "datetimeLocal" that converts the datetime column to Honolulu/Pacific time (*HINT*: look at the lubridate function called `with_tz()`). Then, use dplyr to calculate the mean hourly temperature each month using the temp_C_2m column and the datetimeLocal columns. (*HINT*: Look at the lubridate functions called `month()` and `hour()`). Finally, make a ggplot scatterplot of the mean monthly temperature, with points colored by local hour.
...
mloa2 = mloa %>%
  # Remove NA's
  filter(!(rel_humid %in% c(-99.9, -999.9))) %>%
  filter(!(temp_C_2m %in% c(-99.9, -999.9))) %>%
  filter(!(windSpeed_m_s %in% c(-99.9, -999.9))) %>%

...
set.seed(15)
hw2 <- runif(50, 4, 50)
hw2 <- replace(hw3, c(4,12,22,27), NA)
hw2
```

*PROBLEM 1:*

Take you `hw2` vector and removed all the NAs then select all the numbers between 14 and 38 inclusive, call this vector `prob1`. 
...
prob1 <- hw3[!is.na(hw2)] #removing the NAs
...
Load your `survey` data frame with the read.csv() function. Create a new data frame called `surveys_base` with only the species_id, the weight, and the plot_type columns. Have this data frame only be the first 60 rows. Convert both species_id and plot_type to characters. Remove all rows with weight == NA.
...
### <small>Week 9</small>
*Need a function writing and/or iteration assignment*

...
Let’s plot some Mauna Loa data, downloded from the R-DAVIS github. First, load in the data and remember to load the tidyverse package:

```{r, eval=T, echo=F, warning=F, message=F}
library(tidyverse)
library(lubridate)

mloa <- read_csv("https://gge-ucd.github.io/R-DAVIS/data/mauna_loa_met_2001_minute.csv")
```


With this mloa dataframe, remove the NAs (here designated with -99.9 and -999.9) in rel_humid, temp_C_2m, and windSpeed_m_s. Next, use dplyr to calculate the mean monthly temperature using the temp_C_2m column and the datetime column. (*HINT: Look at the lubridate function called month()). Finally, make a ggplot of the mean monthly temperature.

...
Create a new RMarkdown document. To create an RMarkdown document that we can view on GitHub, your YAML header should look like this:
...
You should download the [flights](data/nyc_13_flights.csv), [planes](data/nyc_13_planes.csv), and [weather](data/nyc_13_weather.csv) CSV files.
...
When you are finished with this assignment and ready to push to GitHub, make sure you push the `.Rmd` file, the `.md` file, and the **folder** containing relevant files that shares the same name as your `.Rmd file`. The `.md` file and the folder will be created after the first time you `knit` your document (so you don’t have to create these, R will do it for you).

Remember! Any changes you make, or files you add (e.g. pictures) need to also exist on GitHub. So if you add a file to your RMarkdown, remember you need to push the folder that contains that file as well, otherwise Github won’t be able to find it and we won’t be able to see it! 

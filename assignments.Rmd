...
Let’s plot some Mauna Loa data, downloded from the R-DAVIS github. First, you need to read in the data (and remember to load tidyverse and lubridate packages):
...
With this mloa dataframe, remove the NAs (here designated with -99.9 or -999.9) in rel_humid, temp_C_2m, and windSpeed_m_s. Next, use dplyr to calculate the mean monthly temperature using the temp_C_2m column and the datetime column. (*HINT: Look at the lubridate function called month()). Finally, make a ggplot scatterplot of the mean monthly temperature.
...
df <- mloa %>% 
  filter(!rel_humid == -99.9, !temp_C_2m == -999.9, !windSpeed_m_s == -99.9) %>% #removing NAs

...
You'll often see `i` used because it refers to "index". For each index value in the vector 1:10, we'll print that index value. You can use the index value more than once in a loop:
...
What's happening is the value of `i` gets inserted into the code block, the block gets run, the value of `i` changes, and the process repeats. For loops can be a way to explicitly lay out fairly complicated procedures, since you can see exactly where your index value is going in the code.

...
Using the `iris` data frame, create a new variable that categorizes petal length into three groups: small (less than or equal to the 1st quartile), medium (between the 1st and 3rd quartiles), and large (greater than or equal to the 3rd quartile). Explore the iris data using `summary(iris$Petal.Length)`, to look at the spread of that variable.

Then use your function of choice: `ifelse()`, or `case_when()` to make a new variable (`iris$petal.length.cat`) based on the conditions listed above

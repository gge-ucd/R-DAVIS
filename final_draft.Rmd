...
2. Create a figure that has date on the x axis and each day's mean departure delay on the y axis. Plot only months September through December. Somehow distinguish between airline carriers (the method is up to you). 
...
5. Write a second function that converts the 24 hour time codes used in the dataframe to 12 hour time codes (i.e., AM and PM). (hint: you'll again want to use some sort of conditional, logical evaluation to identify AM and PM times, and you'll want to use the paste() function to add 'pm' or 'am' as appropriate.). 
6. Save both functions in a new script called customFunctions.R.
...
#4. Make a function that can: (1) convert hours to minutes; and (2) convert minutes to hours (i.e., it's going to require some sort of conditional setting in the function that determines which direction the conversion is going); use this function to convert departure delay (currently in minutes) to hours and then generate a boxplot of departure delay times , colored by carrier. Adjust the axis so that you don't see the outlier delays of more than 10 hours.
...
ggplot(df, aes(x = dep_delay_hrs, fill = carrier)) +
  geom_boxplot()

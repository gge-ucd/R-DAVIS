...
* Rename the levels of treatment to "H", "L" and "M"
* Now order the levels so that "L" is first, "M" is second, and "H" is third
...
treatment <- factor(c("high", "low", "low", "medium", "high"))
levels(treatment) <- c("H", "L", "M")
treatment <- factor(treatment, levels = c("L", "M", "H"))
treatment
``` 
</details>
</div>
...
Some functions in R will automatically convert character strings to factors. For instance, `read.csv()` is a common function to read CSV files into R, but it will turn any character data into factors. You can use the argument `stringsAsFactors=FALSE` in `read.csv()` to turn off this behavior, keeping your character strings as character strings.

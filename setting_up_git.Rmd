...
## Connecting RStudio to your Repository

You should have a repository in the gge-ucd organization...and it should look something like this:
[`https://github.com/gge-ucd/first_lastname`](https://github.com/gge-ucd/first_lastname). Take a minute to navigate to your repo online and take a look at your `README.md` file.

### Create a Version Controlled RStudio Project
...
Now we'll create an RStudio Project and link our Github repository (following [*Section 13.3* of `happygitwithR`](http://happygitwithr.com/rstudio-git-github.html#clone-the-new-github-repository-to-your-computer-via-rstudio)).
  ...
  - First you'll want to look for the big green "Clone or Download" button. Assuming you've set up your SSH key, make sure you have the **Clone with SSH** version and copy that link (should be  *`git@github.com:gge-ucd/first_lastname.git`*, else use the **`https`** link).
 
![](img/ssh_clone.png)

  - Then in RStudio, start a new Project: `File > New Project > Version Control > Git`. In the “**Repository URL**” paste the URL of your new GitHub repository. If you hit **`Tab`**, the *Project directory name* will automagically populate with the exact name of the repository. This is generally preferred so you can keep track of things more easily. If you want to save your github projects or RStudio projects in one place, you can do so by specifying the subdirectory. See the *Nuts & Bolts* notes below for more info.
  
![](img/newRStudio_git_proj_named.png)

<br>

`r emo::ji("nut_and_bolt")` *__More about github/RStudio projects__: One thing to keep in mind is each github repository should only have **ONE** corresponding Rstudio project file. In addition, you never want to save or move a github repository into another github repository...there are hidden files which track your changes (the version control record), and there's potential for conflict if you try to nest or overwite repos and RStudio projects. Best practice is generally make one folder that is `github` or `Rprojects`, and then each project or repository will have it's own single folder within that general directory.*
  ...
  - This new subdirectory that you specified will be all these things:
    ...
    - an RStudio Project
    - It should contain a `README.md` file with some content that I've created...we'll edit that next.
...
We'll go over this part in class. If you have `ssh` set up and you created your RStudio project, you've done most of the hard work...what remains is:
 ...
 - Test whether we can make changes or **edit** our README on our local copy of the repository (your computer)
 - **`commit`** these changes (make a version control record )
 - **`push`** these change to the `remote` which is the copy of the repository on Github.com.
...
### Edit your README
  ...
  - Edit your README:
    - Add your name
    - Remove the limerick and other bullets and add 1-2 sentences about what you study or what you hope to study.
    - add an "X" inside the **Edited by student** box

### Commit to Git

  - Click on the **Git** tab. When you click on the git tab, you should see the `README.md`, and a `.Rproj` file.
  ...
  - Finally before you close everything, hit the **PUSH** button. This *pushes* the changes to the github repository (you may need to enter your `ssh` password once).
